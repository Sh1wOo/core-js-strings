...
function repeatString(str, times) {
  return str.length < 1 || times < 1 ? '' : str.repeat(times);
}
...
function removeFirstOccurrences(str, value) {
  const res = str.indexOf(value, 0);
  if (str.includes(value)) {
    return str.slice(0, res) + str.slice(res + value.length);
  }
  return str;
}
...
function getCardId(value) {
  ...
  const suitIndex = suits[suit];
  const valueIndex = values.indexOf(card);
